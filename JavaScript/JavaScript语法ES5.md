...
所有 DOM 节点都有 `nodeType` 属性，用以获取其节点类型常量的值，进而判断节点类型。通常的用法是将该值与 `Node.属性常量` 作比较，如：`document.nodeType==Node.DOCUMENT_NODE;`。当然这样写也没问题：`document.nodeType===document.TEXT_NODE;`
